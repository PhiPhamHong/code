...
function CoreAjax(typeAction_) {
    ...
    this.assembly = "Core.Sites.Apps"; // Mặc định
    ...
    this.createRequestUrl = function (method) {
        if (this.assembly == null || this.assembly == "")
            this.assembly = "Core.Sites.Apps";

        return "/Services/Ajax.aspx?_n=" + this.assembly + "&_o=" + encodeURIComponent(this.typeAction) + "&_m=" + method;
    }
    ...
    var DownloadResult = function () {
        ...
        this.start = function () {
            var $this = this;
            $this.doing = true;
            timeout = setTimeout(function () { $this.popup = CoreAlert("Đang thực hiện tải tệp", null, function () { $this.abort(); }); }, 1000);
    ...
    this.download = function (method, data, callback, onerror) {
        return this.downloadHelper(this.createRequestUrl(method), { data: data, httpMethod: "POST", callback: callback, onError: onerror });
    }

    this.downloadHelper = function (url, op) {
        var result = new DownloadResult();
        result.start();
        var option = {
            successCallback: function (url) {
                if (op.callback != null) op.callback(url);
                result.end();
            },
            failCallback: function (r) {
                if (arguments[0] != "") {
                    eval("var res = " + arguments[0]);
                    if (op.onError != null) op.onError(res);
                    else Core.alert2(res.Data.MessageError);
                }
                result.end();
            }
        };
        $.extend(option, op == null ? {} : op);
        result.request = $.fileDownload(url, option);
    ...
    this.msgNotConnected = "Không thể kết nối tới Server. Vui lòng thử lại";

    this.post = function (methodServer, data, callback, onerror, options) {
        return this.doAjax("POST", methodServer, data, callback, onerror, options);
    }
    ...
    this.doAjaxHelper = function (methodAjax, dataType, urlRequest, data, callback, onerror, options) {
        ...
        var ajaxOptions = { url: urlRequest, type: methodAjax, data: dataPost, dataType: dataType };
        if (options.contentType != null && options.contentType != "") ajaxOptions.contentType = options.contentType;
        ...
        request.done(function (res) {
            try {
                if (res.Data != null) {
                    ...
                    else if (res.Data.ErrorType == "NotAuthenException" || res.Data.ErrorTypeInner == "NotAuthenException") {
                    ...
                    else if (res.Data.MessageError != null) // Có lỗi về action trong method gọi đến
                    {
                        ...
                        if (options.onError != null) options.onError(res);
                        return;
                    }
            ...
            if ($this.alertWhenFail) Core.notify($this.msgNotConnected, null, "error");
            if (onerror != null) onerror();
        });

        // Thực hiện hiển thị message, nếu như quá trình đang thực hiện request quá 2s

        if (options.blockOverlay) return request;
        if ($this.userOverlay || options.userOverlay == true) $this.timer = setTimeout(function () {
            var containerOverlay = $this.containerOverlay();
            containerOverlay.append($this.currentOverlay = $this.createOverlay());
        }, 500);

        return request;
    }

    this.createOverlay = function () { return $(this.templateOverlay); };
    this.containerOverlay = function () { return $("body"); }

    this.doPostTimer = function (methodServer, data, timeInterval, process, waiting, endWhen, onerror) { this.clearThread("POST", methodServer); this.doTimer("POST", methodServer, data, timeInterval, null, process, waiting, endWhen, onerror); };
    this.doGetTimer = function (methodServer, data, timeInterval, process, waiting, endWhen, onerror) { this.clearThread("GET", methodServer); this.doTimer("GET", methodServer, data, timeInterval, null, process, waiting, endWhen, onerror); };

    this.doPostTimer2 = function (option) {
        option.methodAjax = "POST";
        this.clearThread("POST", option.methodServer);
        this.doTimer2(option);
    }

    // ajax timer, dùng để thực hiện một phương thức theo một chu kỳ, có tính toán độ trễ qua từng request
    this.doTimer = function (methodAjax, methodServer, data, timeInterval, lastRequest, process, waiting, endWhen, onerror) {
        this.threadDoAjax(methodAjax, methodServer, data, process, lastRequest, timeInterval, waiting, endWhen, onerror);
    }
    this.doTimer2 = function (option) {
        this.doTimer(option.methodAjax,
            option.methodServer,
            option.data,
            option.timeInterval,
            option.lastRequest,
    ...
    this.doPostTimeout = function (methodServer, data, timeInterval, process, onerror) { this.doTimeout("POST", methodServer, data, timeInterval, process, onerror); }
    this.doGetTimeout = function (methodServer, data, timeInterval, process, onerror) { this.doTimeout("GET", methodServer, data, timeInterval, process, onerror); }
    this.doTimeout = function (methodAjax, methodServer, data, timeInterval, process, onerror) { this.clearThread(methodAjax, methodServer); this.doTimer(methodAjax, methodServer, data, timeInterval, null, process, null, function () { return true; }, onerror); };
    ...
    this.threadDoAjax = function (methodAjax, methodServer, data, process, lastRequest, timeInterval, waiting, endWhen, onerror) {
        ...
        if (waiting != null && waiting()) {
            ...
            $this.smoothingThread(lastRequest, methodAjax, methodServer, data, timeInterval, process, waiting, endWhen, onerror);
            return;
        }
        ...
        $this.threadTimerAjax[methodAjax + "_" + methodServer] = $this.doAjax(methodAjax, methodServer, data, function (res) {
            ...
            if (endWhen != null && endWhen(res)) { $this.clearThread(methodAjax, methodServer); return; };
            ...
            function (res) {
                ...
                // Có lỗi vẫn tiếp tục chạy cho đến khi kết thúc
                // nếu như có function để kiểm tra việc kết thúc thread thì gọi đến nó
                if (endWhen != null && endWhen(res)) { $this.clearThread(methodAjax, methodServer); return; };
    ...
    this.smoothingThread = function (lastRequest, methodAjax, methodServer, data, timeInterval, process, waiting, endWhen, onerror) {
        var $this = this;
        ...
        $this.threadTimer[methodAjax + "_" + methodServer] = setTimeout(function () { $this.doTimer(methodAjax, methodServer, data, timeInterval, lastRequest, process, waiting, endWhen, onerror); }, ti);
    }
...
CoreAjax.buildType = function (typeAction) {
    var result = {}

    var ta = typeAction.split(',');
    if (ta.length > 0) result.type = ta[0];
    if (ta.length > 1) result.assembly = ta[1];
    if (result.assembly == null || result.assembly == "") result.assembly = "Core.Sites.Apps";
    result.type = encodeURIComponent(result.type);
