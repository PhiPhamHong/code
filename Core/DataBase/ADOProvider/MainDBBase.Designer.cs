...
namespace Core.DataBase.ADOProvider
{
    ...
    public partial class MainDBBase
    {
        ...
        private Pair<T, Param> Execute<T>(string sql, Param paramInputs, Param paramOutputs, Func<DbCommand, T> func, CommandType type = CommandType.StoredProcedure)
        {
            ...
            {
                // Tạo Command
                cmd = CreateCommand(type, sql, paramInputs, paramOutputs);
        ...
        private DataSet CmdFillDataSet(DbCommand cmd)
        {
            ...
            // Nếu có transaction
            if (transaction.IsNotNull()) Db.LoadDataSet(cmd, ds, new string[] { "Sh" }, transaction);

            // Nếu không transaction
            else Db.LoadDataSet(cmd, ds, "Sh");
        ...
        {
            // Nếu có transaction
            if (transaction.IsNotNull()) return Db.ExecuteNonQuery(cmd, transaction);

            // Nếu không transaction
            else return Db.ExecuteNonQuery(cmd);
        }
        ...
        {
            // Nếu có transaction
            if (transaction.IsNotNull()) return Db.ExecuteReader(cmd, transaction);

            // Nếu không có transaction
            else return Db.ExecuteReader(cmd);
        }
