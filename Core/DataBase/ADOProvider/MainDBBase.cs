...
namespace Core.DataBase.ADOProvider
{
    ...
    public static class MainDBBaseExtension
    {
        public static Database GetDatabase(string prefix)
        {
            var encodeConnectionString = ConfigurationManager.AppSettings["EncodeConnectionString" + prefix].WhenNull(() => "False").To<bool>();
            var provider = ConfigurationManager.AppSettings["MainDb_Provider" + prefix];
            string connectionString = string.Empty;
            if (provider.IsNotNull())
            {
                var type = Assembly.Load(provider).GetTypes().FirstOrDefault(t => t.HasInterface(typeof(IDbConnectionString)));
                connectionString = type.CreateInstance<IDbConnectionString>().ConnectionString;
                // Core.Forms

            }
            else connectionString = ConfigurationManager.AppSettings["MainDb" + prefix];

            if (encodeConnectionString) connectionString = connectionString.Decrypt();
