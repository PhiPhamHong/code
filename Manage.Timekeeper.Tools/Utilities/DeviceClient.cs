...
namespace Manage.Timekeeper.Tools.Utilities
{
    public class DeviceClient : IZKEM
    {
        Action<object, string> RaiseDeviceEvent;
        public DeviceClient(Action<object, string> RaiseDeviceEvent)
        ...
        public bool BatchUpdate(int dwMachineNumber) => objCZKEM.BatchUpdate(dwMachineNumber);
        public bool Beep(int DelayMS) => objCZKEM.Beep(DelayMS);
        public bool BeginBatchUpdate(int dwMachineNumber, int UpdateFlag) => objCZKEM.BeginBatchUpdate(dwMachineNumber, UpdateFlag);
        public bool ClearData(int dwMachineNumber, int DataFlag) => objCZKEM.ClearData(dwMachineNumber, DataFlag);
        public bool ClearGLog(int dwMachineNumber) => objCZKEM.ClearGLog(dwMachineNumber);
        public bool Connect_Net(string IPAdd, int Port)
        {
            if (objCZKEM.Connect_Net(IPAdd, Port))
            {
                if (objCZKEM.RegEvent(1, 32767))
                {
                    ...
                    objCZKEM.OnAttTransactionEx += new _IZKEMEvents_OnAttTransactionExEventHandler(DeviceClient_OnAttTransactionEx);
                }
                return true;
            }
            return false;
        ...
        private void DeviceClient_OnAttTransactionEx(string EnrollNumber, int IsInValid, int AttState, int VerifyMethod, int Year, int Month, int Day, int Hour, int Minute, int Second, int WorkCode)
        { }
        ...
        public bool DelUserTmp(int dwMachineNumber, int dwEnrollNumber, int dwFingerIndex) => DelUserTmp(dwMachineNumber, dwEnrollNumber, dwFingerIndex);
        public bool DisableDeviceWithTimeOut(int dwMachineNumber, int TimeOutSec) => objCZKEM.DisableDeviceWithTimeOut(dwMachineNumber, TimeOutSec);
        public bool EnableDevice(int dwMachineNumber, bool bFlag) => objCZKEM.EnableDevice(dwMachineNumber, bFlag);
        public bool GetDeviceTime(int dwMachineNumber, ref int dwYear, ref int dwMonth, ref int dwDay, ref int dwHour, ref int dwMinute, ref int dwSecond) => objCZKEM.GetDeviceTime(dwMachineNumber, ref dwYear, ref dwMonth, ref dwDay, ref dwHour, ref dwMinute, ref dwSecond);
        public bool GetUserInfo(int dwMachineNumber, int dwEnrollNumber, ref string Name, ref string Password, ref int Privilege, ref bool Enabled) => GetUserInfo(dwMachineNumber, dwEnrollNumber, ref Name, ref Password, ref Privilege, ref Enabled);
